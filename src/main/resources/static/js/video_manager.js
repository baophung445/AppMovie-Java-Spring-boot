		...
		function() {
			...
			$('#tableId').on(
					...
					function(e) {
						$(e.target).closest('tr').find(
								'input[name="chosenFilm"]').prop("checked",
								true);
					});

			$('#videoManager').addClass('current-menu-item');
			...
			$('#searchCondition').keyup(
					function(e) {
						...
						if (e.keyCode == 13) {
							window.location
									.replace('video-manager?searchCondition='
											+ searchCondition);
						}
					});
			...
			$('#btnEdit').click(function() {
				...
				var filmId = $('input[name="chosenFilm"]:checked').val();

				var newFilmName = $('#newFilmName').val();
				var newYear = $('#newYear').val();
				var newLength = $('#newLength').val();
				var newIntroduction = $('#newIntroduction').val();
				var newDeployStatus = $('#newDeployStatus').val();
				var newRequestStatus = $('#newRequestStatus').val();
				var newEpisodeCount = $('#newEpisodeCount').val();
				var newRequestEmail = $('#newRequestEmail').val();
				var newAcceptAccount = $('#newAcceptAccount').val();

				var token = $('#_csrf').attr('content');
				var header = $('#_csrf_header').attr('content');
				var webDomain = $("#webDomain").val();

				if (newFilmName == '' || newFilmName == null) {
				...
				if (newDeployStatus == '' || newDeployStatus == null) {
					alert('Deploy status is invalid');
					return;
				}
				if (newRequestStatus == '' || newRequestStatus == null) {
					alert('Request status is invalid');
					return;
				}
				if (newEpisodeCount == '' || newRequestStatus == null) {
					alert('Request status is invalid');
					return;
				}
				if (newRequestEmail == '' || newRequestEmail == null) {
					alert('Request email is invalid');
					return;
				}
				if (newAcceptAccount == '' || newAcceptAccount == null) {
				...
				$.ajax({
					...
					url : webDomain + 'member/editVideo/' + filmId,
...
function onclickFilmDetail() {
	...
	if (typeof index == 'undefined') {
		...
		window.location.replace('member/videoDetail/' + index + '?searchCondition=');
	}
}
function searchFilm() {
	var searchCondition = $('#searchCondition').val();

	window.location.replace('video-manager?searchCondition=' + searchCondition);
}